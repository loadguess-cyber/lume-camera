...
> ## 종료된 프로젝트입니다 (2026-07)
>
> 더 개발하지 않습니다. 이슈·PR 도 받지 않습니다.
> 코드는 지우지 않고 그대로 두니 필요한 부분은 가져다 쓰셔도 됩니다.
>
> **왜 접었나** — 앱의 전제가 "카메라 미리보기에 프리셋을 실시간으로 얹어서 그대로 찍는다" 였는데,
> 웹뷰 카메라로는 그 결과물이 기본 카메라 앱의 화질을 따라갈 수 없었습니다. RAW 를 받을 수 없고,
> 대부분의 기기에서 ISO·셔터를 잡을 수 없고, 멀티프레임 HDR 도 못 씁니다.
> 색은 원하는 대로 나와도 사진 자체가 기본 카메라보다 나빠지는데, 그러면 찍을 이유가 없어집니다.
> 이건 코드를 더 써서 넘을 수 있는 벽이 아니라 웹뷰라는 선택에 딸려온 천장이었습니다.
> 도중에 방향을 튼 게 [LUME Studio](#lume-studio--프리셋-중심-사진-편집--일괄-처리) 였고,
> 이미 찍힌 사진에 프리셋을 먹이는 쪽은 실제로 잘 돌아갑니다. 다만 그건 처음 만들려던 물건이
> 아니었고, 그 자리에는 이미 좋은 도구가 많아서 여기서 멈추기로 했습니다.
>
> **가져다 쓸 만한 것** — 이 저장소에서 값이 나가는 부분은 카메라가 아니라 색 엔진입니다.
> 둘 다 외부 라이브러리 없이 단독으로 동작하고, 카메라 코드와 얽혀 있지 않습니다.
>
> | 파일 | 내용 |
> |---|---|
> | `www/js/gl.js` | 라이트룸 현상 파이프라인을 근사한 WebGL 셰이더. 톤커브·HSL 8채널·컬러그레이딩·부분대비·안개제거까지 |
> | `www/js/xmp.js` | 라이트룸 `.xmp` 프리셋 파서 겸 생성기. 읽어들인 프리셋을 다시 내보내면 라이트룸에서 그대로 열립니다 |
> | `www/js/presets.js` | 내장 프리셋 9종의 파라미터 값 |
> | `www/js/zip.js` | 의존성 없는 ZIP 작성기 (무압축 저장) |
>
> 엔진이 어떻게 먹는지는 `www/try.html` 을 열어보면 카메라 권한 없이 바로 확인됩니다.
>
> 아래 문서는 개발이 멈춘 시점의 내용을 그대로 둔 것이라 현재형으로 쓰여 있습니다.
